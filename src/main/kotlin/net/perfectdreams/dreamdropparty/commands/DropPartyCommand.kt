...
class DropPartyCommand(val m: DreamDropParty) : SparklyCommand(arrayOf("dropparty")) {
    ...
    fun start(sender: Player) {
        if (m.dropParty.running) {
            sender.sendMessage("${DreamDropParty.PREFIX} Já existe um evento Drop Party acontecendo!")
            return
        }
